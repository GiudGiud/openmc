  ...
  if (material() != MATERIAL_VOID) {
    if (settings::run_CE) {
      if (material() != material_last() || sqrtkT() != sqrtkT_last() ||
        ...
        std::cout << "Skipping evaluation " << sqrtkT() << " " << material()
                  << std::endl;
      }
    } else {
...
void Particle::event_advance()
{
  ...
  std::cout << "seeds " << seeds()[0] << " " << seeds()[1] << " " << seeds()[2]
            << " " << *current_seed() << std::endl;
  ...
  if (type() == ParticleType::electron || type() == ParticleType::positron) {
    ...
    const auto rand = prn(current_seed());
    // std::cout << "RANDOM " << rand << std::endl;
    collision_distance() = -std::log(rand) / macro_xs().total;
  }
